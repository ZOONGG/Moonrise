...
public sealed class SupportStage1Tests
{
    ...
    public void SupportUiUsesOwnedControlsAndDoesNotTiePollingToWindowFocusOrTelegramLaunch()
    {
        ...
        Assert.DoesNotContain("SupportCryptoAmountText", xaml, StringComparison.Ordinal);
        Assert.DoesNotContain("Открыть в кошельке", code, StringComparison.Ordinal);
        Assert.DoesNotContain("Сумма скопирована", code, StringComparison.Ordinal);
        Assert.DoesNotContain("hasWalletDeepLink", code, StringComparison.Ordinal);
        Assert.Contains("<ColumnDefinition Width=\"48\"/>", xaml, StringComparison.Ordinal);
        Assert.Contains("Width=\"36\" Height=\"36\"", xaml, StringComparison.Ordinal);
